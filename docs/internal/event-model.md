...
1. **持久化与 wire 解耦**。[`storage-jsonl.md`](./storage-jsonl.md) 计划把 `AgentEvent` 投影成 jsonl journal，可 resume。如果落盘格式 == ACP wire，ACP crate 升级 = on-disk schema 变 = 老会话不能 resume。让 storage 有自己的 `SessionRecord` 表示，跟 wire 无关，存档稳定性独立掌控。
...
- `defect-storage` 投影到恢复日志前可以复用这些字段，避免主循环再维护一套中间 wire 类型
...
- **storage raw 审计**：如果开启原始事件日志，可以保留它们辅助排障；恢复日志不依赖这些事件
...
| 变体 | wire (ACP) | storage journal | tracing |
| --- | --- | --- | --- |
| `TurnStarted` | — | → `SessionRecord::TurnStarted` | ✓ |
| `TurnEnded` | → `PromptResponse` | → `SessionRecord::TurnEnded` | ✓ |
| `AssistantText` | → `AgentMessageChunk` | 累加后 → assistant `Message` | ✓ (debug) |
| `AssistantThought` | → `AgentThoughtChunk` | 累加后 → assistant `Message` | ✓ (debug) |
| `ToolCallStarted` | → `ToolCall` | 累加后 → assistant `MessageContent::ToolUse` | ✓ |
| `ToolCallProgress` | → `ToolCallUpdate` | — | ✓ (debug) |
| `ToolCallFinished` | → `ToolCallUpdate` | 累加后 → user `MessageContent::ToolResult` | ✓ |
| `PolicyDecision (Allow/Deny)` | — | — | ✓ |
| `PolicyDecision (Ask)` | → `RequestPermissionRequest` | — | ✓ |
| `PermissionResolved` | — (是 ACP request 的返回，不再发) | — | ✓ |
| `LlmCallStarted` / `Finished` | — | — | ✓ |
| `ContextCompressed` | — | —；压缩结果由 storage 写 `Snapshot` | ✓ |

...
`web_search` 是当前唯一落地的 capability；后续 `image_generation` / `code_execution` / `computer_use` 等同形态能力沿用本设计。本文沉淀 capability 的层级、与 [`Tool`] / [`LlmProvider`] / 配置的边界，以及 P1 的运行时装配规则。
...
1. **能力 ≠ 工具**——provider-hosted 能力在 wire 上由 provider 自己执行；本地工具走 [`Tool`] trait。能 capability 化的是前者，因为它无法被 [`Tool`] trait 干净表达；能用 [`Tool`] trait 完整表达的（`fs` / `bash` / `fetch` / 本地 `search`）就不进 capability 层。
2. **session 启动期一次性裁决**——`(provider, mode)` 在 session 生命周期内不变，turn loop 不重判。
3. **provider-hosted 能力不实现 [`Tool`] trait**——hosted web_search 由 provider 自己执行、agent 无法逐次拦截，强行套 [`Tool`] 会扭曲 stop reason / 审批 / 事件流语义。

> **`web_search` 与本地 `search` tool 是两件不同的事。**
> - `web_search`（capability）：互联网检索，由 provider hosted 实现（Anthropic `web_search_*` / OpenAI Responses `web_search`）。本文管这一份。
> - `search`（[`Tool`]）：工作区内的 grep / glob 检索，纯本地实现，不进 capability 层。配置入口 `[tools.search]`，详见 [tools-search.md](./tools-search.md)。
>
> 两者**完全独立**：可以同时启用，LLM 会同时看到 hosted `web_search` 与本地 `search` 两个工具——一个搜外网，一个搜代码库，不存在能力竞争。
...
┌───────────────────────────────────────────────────────────┐
│  capability 层                                            │
│  ─ 回答："这个 session 有 web_search 能力吗？"            │
│  ─ 配置入口：[capabilities.web_search]                    │
│  ─ 类型：WebSearchCapabilityMode { Delegate, Disabled }   │
└───────────────────────────────────────────────────────────┘
                ↓ session 启动期裁决
┌───────────────────────────────────────────────────────────┐
│  装配层                                                   │
│  ─ ResolvedSessionCapabilities                            │
│  └─ hosted: HostedCapabilities { web_search: bool }       │
└───────────────────────────────────────────────────────────┘
                ↓
┌──────────────────────┐  ┌─────────────────────────────────┐
│  provider 层         │  │  工具层                          │
│  ─ 暴露 hosted       │  │  ─ 本地 search tool（grep/glob）│
│    web_search 给 wire│  │  ─ fetch / fs / bash / ...      │
└──────────────────────┘  └─────────────────────────────────┘
```

| 层 | 职责 | 不负责 |
|---|---|---|
| capability | 决定 hosted 能力是否暴露 | 怎么执行 / 本地工具的开关 |
| provider | 实现状态自报家门 + hosted lower 到 wire | 产品语义 / 来源选择 |
| 工具 | 本地真正执行的工具（含本地 `search`） | hosted 能力 |
...
TOML 串：`"delegate"` / `"disabled"`。

语义对照：

| 模式 | hosted web_search | MCP 同名 `web_search` |
|---|---|---|
| `Delegate` | ✅ 暴露 | 走 `mcp.<server>.web_search`（§6） |
| `Disabled` | ❌ 不暴露 | 走 `mcp.<server>.web_search` |

> 本地 `search` tool 不在本表中——它由 `[tools.search].enabled` 单独控制，**与本 mode 无关**。

为什么不做 `prefer_provider` / `require_provider` 这类 fallback 模式：

1. 「能力来源选择」与「失败回退策略」混在一起会让配置语义可预测性变差
2. 静默回退会改变行为可观测性
3. 二态先收敛实现与测试矩阵；P1 不做隐式回退

---

## 4. `HostedCapabilities`：provider 自报家门

[`LlmProvider`] trait 上挂的方法：
...
impl HostedCapabilities {
    ...
    pub const fn with_web_search(web_search: bool) -> Self { Self { web_search } }
}
```
...
| `HostedCapabilities` | adapter 实现状态（能否在 wire 上声明 hosted web_search） | adapter 自己声明 |

各 provider 当前实装：

| provider | `hosted_capabilities().web_search` | 说明 |
|---|---|---|
| Anthropic | `false`（P1） | 待接 `web_search_*` hosted tool |
| OpenAI | `false`（P1） | 待接 Responses API `web_search` |
| DeepSeek | `false` | 不支持 hosted web_search |
| Echo | `false` | 测试 stub |
...
- agent 层不感知版本——`HostedCapabilities { web_search: bool }` 只回答 yes / no
...
pub struct ResolvedSessionCapabilities {
    /// 最终决定本 session 是否走 hosted web_search。
    /// `Delegate × supported` → `web_search = true`；其余情况 → `false`。
...
- session 内任意 turn 看到的 hosted 来源都一致

### 5.2 裁决表

| `mode` | provider 支持 hosted web_search | 装配结果 |
|---|---|---|
| `Delegate` | 支持 | `hosted.web_search = true` |
| `Delegate` | 不支持 | session 启动失败，返回 [`SessionInitError::CapabilityUnsatisfied`](#53-启动失败) |
| `Disabled` | 任意 | `hosted.web_search = false` |
...
`Delegate` 是用户的显式选择「我要 hosted」。provider 不支持时静默 fallback 到禁用会改变行为可观测性，静默不暴露会让模型幻觉调用一个不存在的能力——两者都比 fail-fast 差。

错误信息内嵌 actionable hint：

```text
web_search capability is unsatisfied: provider `deepseek` does not support hosted web_search.

To fix this, choose one of:
  1. Disable hosted web_search for this provider:
       [providers.deepseek.capabilities.web_search]
       mode = "disabled"
  2. Change global default to `disabled` and only delegate where supported:
       [capabilities.web_search]
       mode = "disabled"
       [providers.<hosted-supported>.capabilities.web_search]
       mode = "delegate"
```
...
`web_search` 在任一 turn 内只有一个 hosted 来源：要么 provider hosted，要么不暴露。本地 `search` tool 不在 capability 层，因此不会与 hosted `web_search` 撞名（一个叫 `web_search`，一个叫 `search`），LLM 同时看到两个工具但语义清晰。
...
provider-hosted web_search 与本地 [`Tool`] 工具的底层执行路径不同，不需要强行抹平：

- provider-hosted web_search 走 provider response item 路径
- 本地 `search` tool / 其它工具走本地 [`Tool`] 路径

### 7.2 上层语义统一为 ACP `ToolCallUpdate`

虽然底层分轨，hosted web_search 由 provider adapter 在解析流时翻译成 ACP `ToolCallUpdate { kind: ToolKind::Search, ... }`，复用 ACP 已有的 `Search` 工具种类。
...
agent 主循环在收到 hosted web_search 的 ACP 推送后，**不**像本地工具那样需要等 `tool_call_update.status = Completed` 才进入下一轮——provider stream 自己会带回 result，turn loop 不阻塞在 hosted web_search 上。

### 7.3 `MessageContent::ProviderActivity`

hosted web_search 的调用与结果**进入 history**（否则跨轮不可见，模型会重复发起同样的搜索），但用专属 `MessageContent` variant，不复用 `ToolUse / ToolResult`：
...
| **不持久化到磁盘** | `#[serde(skip)]` 或在持久化 codec 里显式丢弃。session resume 后如果模型再次触发 hosted web_search，会重新发起一次新调用，不依赖旧 payload |
...
```rust
// 不会出现：
impl Tool for HostedWebSearchAdapter { ... }    // ✗ 拒绝
```
...
### 8.2 本地 `search` tool 是普通 [`Tool`]，不在本文管辖

本地 `search`（grep/glob）走 [`Tool`] trait，与本 capability 层无关。它的开关 / 参数 / schema 全部由 `[tools.search]` 段控制，详见 [`tools-search.md`](./tools-search.md)。

[`Tool`] trait 当前承载：`fetch` / `fs` / `bash` / 本地 `search`。
...
- 看到 `hosted_capabilities.web_search = true` 时，在 wire 上声明 hosted web_search tool（具体名字 / 版本由 adapter 决定）
- 看到 `false` 时，wire 上不声明 hosted web_search——即便 provider 服务端支持，也不暴露给模型
...
| 部件 | 状态 |
|---|---|
| [`WebSearchCapabilityMode`] / [`SessionCapabilitiesConfig`] | ✅ |
...
| Anthropic / OpenAI hosted web_search wire 编解码 | ❌（adapter 仍返回 `web_search: false`） |
| 本地 `search` tool 实现（独立于本 capability） | ✅（[`crates/tools/src/search.rs`](../../crates/tools/src/search.rs)，详见 [tools-search.md](./tools-search.md)） |
| 切 provider 时 history 降级 | ❌（`Session` 尚无 `set_provider`） |

[`WebSearchCapabilityMode`]: ../../crates/agent/src/session/capabilities.rs
...
- `Delegate × supported` → `hosted.web_search = true`
- `Delegate × unsupported` → `SessionInitError::CapabilityUnsatisfied`
- `Disabled × *` → `hosted.web_search = false`
- 错误消息含 `[providers.<p>.capabilities.web_search]` 与 `[capabilities.web_search]` 两条 hint
...
### 12.1 把 hosted web_search 继续伪装成 [`Tool`]
...
### 12.2 把本地 `search`（grep/glob）也塞进 capability 层

拒绝原因：

- 本地 `search` 已经能用 [`Tool`] trait 完整表达，不需要额外抽象
- 把 hosted web_search 与本地 grep 捏成一个 capability 会让命名混淆、配置语义不清——一个搜外网，一个搜代码库，本就是两件事
- 强耦合会带来「hosted 启用就关闭本地 search」的诡异行为，与「能力越多越好、来源越清晰越好」的直觉相反

### 12.3 让 hosted/local web_search 同时暴露给模型

（暂未实装本地 web_search，所以本条目前不适用——若未来引入「本地 web_search 工具」再单独评估。）

### 12.4 把 `fetch` 也提升为 capability
...
1. `web_search` 是 capability，二态：`Delegate` / `Disabled`，hosted-only
2. 本地 `search`（grep/glob）走 [`Tool`] trait，由 `[tools.search]` 单独管理，与 capability 层无关
3. provider-hosted 与 local tool 分轨，**不**共用执行路径与 [`Tool`] trait
4. session 启动期一次性裁决，不满足直接 fail
5. MCP 工具一律 `mcp.<server>.<name>` 注册
6. hosted activity 进 history 但不持久化、不上抬 ACP transcript
7. fetch 走 [`Tool`] 而不是 capability——参见 [`tools-fetch.md`](./tools-fetch.md)

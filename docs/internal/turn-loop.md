...
pub(crate) struct TurnConfig {
    ...
    /// 触发 History::compact 的 token 阈值。`None` 表示从不主动压缩。
...
async fn run_tools_concurrently(approved: Vec<Approved>) -> Vec<ToolResult> {
    ...
    if let Some(limit) = NonZeroUsize::new(config.max_concurrent_tools) {
        // TODO: 如有需要在这里改成 Semaphore 限制；v0 默认不限。
    }
...
```rust
async fn maybe_compact(state: &mut TurnState) -> Result<(), TurnError> {
    let Some(threshold) = config.compact_threshold_tokens else { return Ok(()) };
    let Some(estimate) = history.token_estimate() else { return Ok(()) };
    if estimate < threshold { return Ok(()); }

    let report = history.compact().await.map_err(TurnError::Internal)?;
    emit ContextCompressed {
...
- `compact_threshold_tokens = None`：永不主动压缩（v0 默认行为，因为 `VecHistory::token_estimate` 返回 `None`）
- `History::token_estimate` 返回 `None` 时不触发压缩——provider 端要么自己处理、要么直接报 `ContextLengthExceeded` 进 retry 路径
...
| `History::compact` 失败 | `Err(TurnError::Internal(_))` | §4 |
...
| `compact_threshold_tokens` | `None` | v0 `VecHistory` 无 token 估算，主动压缩留给 v1 |
| `max_llm_retries` | `3` | provider transient error 默认 3 次 |
| `max_concurrent_tools` | `0`（不限） | v0 不加 Semaphore |

...
1. **解耦 cli 装配**。cli 的 `bin/cli.rs` 是"拼装样板"，二次开发者照着改。把可观测性
   的初始化收敛到一个 `defect_obs::init(...)` 入口，cli 只调一行，后续加
...
3. **generation `input` = 完整 prompt。** `LlmCallStarted` 携带 `LlmRequestSnapshot`
   ...
   **system 作第一条 `{role:"system"}`**。这是 Langfuse SDK 的标准 input 格式。

4. **thinking 放 `metadata.reasoning`**（ingestion 无专用 reasoning 字段），不并进 output。

5. **`UserPromptCommitted` 先于 `TurnStarted`**：projector 用 `pending_input` 暂存，
   `TurnStarted` 建 `TurnState` 时取走、随 `trace-create` 即写 input（不必等 TurnEnded）。

6. **207 不是错误**：ingestion 端点对 batch 请求**始终返回 207**，逐条结果在 body 的
   `successes`/`errors`。只有 `errors` 非空才告警（`ingest.rs::inspect_response`）。

### 3.3 usage 字段映射

defect 的 `Usage`（`crates/agent/src/llm/chunk.rs:76`）→ Langfuse `usageDetails`（自由 map，
键名自定，`total` 不填则后端推导）。实现见 `projector.rs::usage_to_details`：

| defect `Usage` | `usageDetails` 键 |
|---|---|
| `input_tokens` | `input` |
...
各字段 `Option<u64>`，`None` 视为缺省不上报（不写 0，避免污染聚合）。usage 写在 **trace 的
`metadata.usage`**（turn 级，准确），generation 级无可靠 usage（见 §3.2.1）。
...
**具体实现**在 `defect-obs`。否则 `defect-agent` 会倒挂依赖 observability crate。
...
- `types.rs`：`TracingConfig` 加 `langfuse: Option<LangfuseConfig>`；新增
  ...
  + `LangfuseSection`（`#[serde(deny_unknown_fields)]`，对齐其它 section）。
- `loader.rs`：投影 section → 有效配置。
- **凭据明文存放**：`public_key` / `secret_key` 是明文字符串（无 env 占位展开、无
  `_env` 变体）。放用户级（`~/.config/defect/config.toml`）或项目本地
  （`.defect/config.local.toml`，已被 `.defect/.gitignore` 忽略）。
- **关于共享配置的 sanitize**：早期设计想给 `tracing.langfuse` 加项目层 denylist
  （仓库共享配置不许设凭据/host）。但后续 config 重构把整套项目层安全 denylist 删了
  （见 `docs/internal/config.md` §8 / memory `project-config-key-whitelist-tech-debt`，
  宗旨：不替用户做家长式裁剪）。**现状：任何层都可设 langfuse，原样生效**——secret 不进
  仓库靠 `.gitignore`，不靠 config 层拦截。
...
- **OTLP 导出**：`OtlpTracingConfig` 脚手架已在，未来在本 crate 加 OTLP 导出，与 Langfuse
  ...
  **OTLP 阶段的实现路径**：那时应走 `tracing` → `tracing-opentelemetry` Layer → OTLP
  exporter，即用 `tracing-subscriber` 收集 span。**但 native ingestion 阶段不该这么做**——
  现有的 `llm_call` / `acp_prompt_turn` span 只带 vendor/model/attempt，**不含 messages /
  usage / output**（那些只在 `AgentEvent` 流里），改造成本不比走 `AgentEvent` 低，还丢掉
  可丢弃降级控制。结论：native 阶段消费 `AgentEvent`（现状），OTLP 阶段才用 tracing Layer。
- **工具内 embedding 透传**（§5 的 `ObsContext`/`ObsSink`）：本期未做，按需。
...
- **`Message` / `system` 的 `Arc` 化**（已落地，非二期）：`Message.content: Arc<[MessageContent]>`、
  `CompletionRequest.system: Option<Arc<str>>`、`Turn.system_prompt: Option<Arc<str>>`。动机：
  `EventEmitter::emit` 对每个订阅者 clone 一次事件、`provider.complete(req.clone())` 也整体 clone，
  长上下文下深拷贝代价高；`Arc` 让 clone 退化成引用计数。开了 serde `rc` feature（`Arc<[T]>`/
  `Arc<str>` 序列化成 array/string，落盘格式与 `Vec`/`String` 一致、向后兼容）。这是接入 langfuse
  时顺手还掉的既有性能债，非 langfuse 专属。

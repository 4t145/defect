...
| `terminal/*`                                            | request      | agent → client     | ✓    | 走 [`acp-shell.md`](./acp-shell.md)；按客户端 `terminal` 能力位决定 `AcpShellBackend` 还是 `LocalShellBackend` |
...
- `terminal/*` —— 由 [`acp-shell.md`](./acp-shell.md) 落地：客户端声明 `terminal: true` 时走 `AcpShellBackend`，否则降级到 `LocalShellBackend`
